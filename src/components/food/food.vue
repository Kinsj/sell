...
<script type="text/ecmascript-6">
  ...
  import Vue from 'vue';
  import {formatDate} from '../../common/js/date'; // import js文件公用函数的时候需要这样写
  ...
  export default {
    ...
    methods: {
      ...
      toggleContent() {
        ...
        this.$nextTick(() => { // 刚改变这几个属性的时候dom还没有更新，立即执行scroll.refresh()是没有意义的,所以需要异步到nextTick中执行
          this.scroll.refresh(); // 刷新可滚动的范围,因为改变后内容会增加或减少，需要同步可滚动的范围
        });
      },
      needShow(type, text) {
...
<style lang="stylus" rel="stylesheet/stylus">
  ...
  .food
    ...
    .image-header
      ...
      width: 100%
      height: 0
      padding-top: 100% // 此处设置padding-top 100%可以保证盒子与宽相等
      img
        position: absolute
        top: 0
        left: 0
